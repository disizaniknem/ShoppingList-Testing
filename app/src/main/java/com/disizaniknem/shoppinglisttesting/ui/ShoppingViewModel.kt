...
import com.disizaniknem.shoppinglisttesting.others.Constants
import com.disizaniknem.shoppinglisttesting.others.Event
import com.disizaniknem.shoppinglisttesting.others.Resource
import com.disizaniknem.shoppinglisttesting.repositories.ShoppingRepository
    ...
    fun insertShoppingItem(name: String, amountString: String, price: String) {
        ...
        if (name.length > Constants.MAX_NAME_LENGTH) {
            _insertShoppingItemStatus.postValue(
                Event(
                    Resource.error(
                        "The name of the item must not exceec ${Constants.MAX_NAME_LENGTH} characters",
                        null
                    )
                )
            )
        ...
        if (price.length > Constants.MAX_PRICE_LENGTH) {
            _insertShoppingItemStatus.postValue(
                Event(
                    Resource.error(
                        "The price of the item must not exceec ${Constants.MAX_PRICE_LENGTH} characters",
                        null
                    )
                )
            )
